...
const P = OUT;
const PP = OUT;
const at = await page.evaluate(([id, label])=>{ const d=window.W2H.DISTRICTS.find(x=>x.id===id); const pl=(d.places||[]).find(p=>p.label===label); if(!pl) return null; const s=d.dispScale||1, c=Math.cos(d.rot), n=Math.sin(d.rot); return { x: d.x + (pl.x*c + pl.z*n)*s, z: d.z + (-pl.x*n + pl.z*c)*s, y: 2.9 + (pl.h||6)*s*0.4 }; }, [ID, LABEL]);
