...
const ID = process.argv[2] || 'yas'; const KIT = process.argv[3]; const DIST = +process.argv[4] || 260; const ELEV = +process.argv[5] || 120; const ANG = +process.argv[6] || 2.4;
...
const P = OUT;
const PP = OUT;
const at = await page.evaluate((name)=>{ let best=null; window.W2H.scene.traverse(o=>{ if(o.isMesh && o.userData && o.userData.kitName===name && !best){ o.updateMatrixWorld(true); o.geometry.computeBoundingBox(); const bb=o.geometry.boundingBox.clone().applyMatrix4(o.matrixWorld); best={x:(bb.min.x+bb.max.x)/2,z:(bb.min.z+bb.max.z)/2,y:bb.min.y,h:bb.max.y-bb.min.y}; } }); return best; }, KIT);
console.log('kit at', JSON.stringify(at));
if (at){
  await page.evaluate(([l,dist,elev,ang])=>{ const g=window.W2H.goal, c=window.W2H.cur; g.target.set(l.x, l.y + l.h*0.35, l.z); g.dist=dist; g.elev=elev; g.angle=ang; g.fov=42; c.target.copy(g.target); c.dist=g.dist; c.elev=g.elev; c.angle=g.angle; c.fov=g.fov; }, [at, DIST, ELEV, ANG]);
  await page.waitForTimeout(7000); await page.screenshot({ timeout: 150000, path: PP + 'kit-' + KIT + '.png' });
}
