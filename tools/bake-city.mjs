...
const ISLANDS = [
  ...
  /* THE EAST CUT STAYS AT 54.6112, AND THE ATTEMPT TO MOVE IT IS THE REASON THIS NOTE EXISTS.

     Thirty-six venues addressed "Al Raha Beach, Al Zeina" and "Al Raha Blvd" fall east of this cut
     and so outside the modelled land, and pushing the frame east to 54.6210 to take them in looked
     like the obvious fix. It was not, and the failure is specific to noCoastline.

     ON A noCoastline DISTRICT THE FRAME IS THE SHORE wherever OSM has no coastline to clip
     against. Al Raha Beach's crescents are recent reclamation and OSM's coastline there does not
     close, so the widened frame did not admit real land — it INVENTED a rectangle of it, out into
     the Yas channel, straight-edged, with Overture footprints landing on top. Drawn flat by
     tools/bench/plan.mjs beside the same window in a satellite view the difference is not subtle,
     and the owner's words for the result were "buildings where they should not be".

     So the venues stay off-model until the frame can follow a real coast rather than a rectangle,
     which is the same work as the coarse-outline problem this district has anyway: the crescents —
     Al Muneera's spirals, Al Bandar's marina fingers, Al Dana — are drawn here as three or four
     blobs. Widening a frame cannot fix that and can make it worse.

     The southern envelope is untouched, deliberately: Khalifa City's north edge is DERIVED from
     it (see DEFERRED below) and the E10 belongs to Al Raha. */
  { id:'raha', name:'Al Raha', bbox:[24.4200, 54.5600, 24.4600, 54.6150], centre:[24.4460, 54.5850],
    noCoastline: true,
    outlineLL: [
