...
/* ---------------------------------------------------------------------------------------------
   ...
   the villa estates were described by three hand-clustered lowRise blobs on Saadiyat. Circles
   overshoot at the edges, miss the tails, and say nothing about islands nobody has clustered: Hidd
   Al Saadiyat, the whole north-east spit, was never covered by any of them.

   SAME TRICK vacantAt ALREADY USES. Precompute from the baked footprints, ship the answer as a
   table, and the generator can ask a question about real buildings at a moment when no real
   building has arrived.

   32 x 32 over the normalised island square. THE FRAME IS b.x / d.r WITH THE REFITTED r — the one
   from max(extent)/2/M_PER_UNIT, so Corniche is 1,220 units and not the authored 190. Built in the
   authored frame this mask put two thirds of Saadiyat outside its own window and matched a third
   of the villas; built in the right one it matches 93 per cent.

   A CELL IS A VILLA CELL when it holds at least three surveyed footprints whose MEDIAN area is
   under 800 m2 and median height under 12 m. Median and not mean: one school in a street of houses
   moves a mean and does not move a median, which is precisely how the cull-time version mis-fired
   on Al Maryah.

   0 MEANS NO AND ALSO MEANS DO NOT KNOW, and those must stay merged. An unsurveyed cell keeps the
   perimeter block it has always had, because filling ranks on evidence we do not have is how the
   black rectangles arrived in the first place.

   ONE DILATION PASS, INTO UNKNOWN CELLS ONLY, never overriding a measured non-villa cell: a thin
   patch of survey inside an estate is a gap in the data, not a change of land use.

   Match rate against the villa-scale footprints of each island — Saadiyat 93%, Yas 96%, Al Reem
   77%, Corniche 67%, Al Maryah 57%. The last two are low because those islands genuinely are
   mixed, which is the mask doing its job rather than failing at it.
   --------------------------------------------------------------------------------------------- */
export const VILLA_N = 32, VILLA_LO = -1.45, VILLA_HI = 1.45;
export const VILLA_MASK = {
  /* corniche: 128 villa cells of 1024 */
  corniche: [
    ...
    '00000000000000000000101110100000',
    '00000000000000000011111101110000',
    '00000000000000111111000001110000',
    '00000000000011111110111111100000',
    '00000000000111111111001111100000',
    '00000000000110111011111111000000',
    '00000001101111110011101100000000',
    '00000110000011111110000000000000',
    '00000110011110001110000000000000',
    '00000110100000011100000000000000',
    '00000011110000001000000000000000',
    '00000001001000010000000000000000',
    '00000000000000000000000000000000',
    '00000000000000000000000000000000',
  ...
  maryah: [
    ...
    '00000000000000000000000000000000',
    '00000000000000000000000000000000',
    '00000000000010000100000000000000',
  ...
  reem: [
    ...
    '00000000000000000110011000000000',
    '00000000010000000110000000000000',
    '00000000000000000000000000000000',
  ...
  /* saadiyat: 101 villa cells of 1024 */
  saadiyat: [
    '00000000000000000000000000000000',
    '00000000000000000000000000000000',
    '00000000000000000000000000000000',
    ...
    '00000011101111111100000000000000',
    '00000011100111111110000000000000',
  ...
  yas: [
    ...
    '00000000000000000000000000000000',
    '00000000000000000000000000000000',
    '00000000000011101110000000000000',
    '00000000000011111110000000000000',
    '00000000000000011110000000000000',
    '00000000000000010110000000000000',
    '00000000000000111100000000000000',
    '00000000000000001110000000000000',
    '00000000000000011111000000000000',
    '00000000000000111111000000000000',
    '00000000000000111111011100000000',
    '00000000000111010011111110000000',
    '00000000011111111101111110000000',
    '00000000111111110110111110000000',
    '00000001111111111111110111000000',
    '00000001111111101111111111000000',
    '00000011111111111111111111000000',
    '00000001111111111110011111000000',
    '00000000000011001110011100000000',
...
export function villaAt(id, nx, ny){
  ...
  if (!g) return false;                        // unlisted island: perimeter model, unchanged
  const s = VILLA_N / (VILLA_HI - VILLA_LO);
  const i = Math.floor((nx - VILLA_LO) * s), j = Math.floor((ny - VILLA_LO) * s);
  if (i < 0 || i >= VILLA_N || j < 0 || j >= VILLA_N) return false;
  return g[j].charCodeAt(i) === 49;            // '1'
}
