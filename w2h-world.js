...
const LM = {
  ...
  /* QASR AL WATAN. The one anchor that needed no solving — it was carried in from an earlier
     session and lands dead on a 453.3 x 405.4 m record with a 118-point polygon, the two 170 m
     flanking palaces either side of it. Kept at the value it arrived with rather than recentred
...
const DISTRICTS = [
  ...
  { id:'saadiyat', name:'Saadiyat',   x:-44*ISLE_SCALE, z:-116*ISLE_SCALE, r:56*ISLE_SCALE, rot: 0.15, tint:0xDDD3C0,
    ...
    built:false, coreN:[0.15, 0.10], coastPark:[0.02, 0.28, 0.070], places:[
      ...
      { label:'Natural History Museum',  x:-405.0, z:255.0, h: 5, r:26 },   // on its two baked footprints south-west of the lagoon (world v308)
      { label:'teamLab Phenomena',       x:-276.0, z:216.0, h: 4, r:22 },   // on the district side of the bridge approach, by the interchange (world v309)
...
const LM_SAADIYAT = {
  ...
  znm:        { x:-371.6, z:177.8 },
  guggenheim: { x:-466.5, z: 86.6 },   // THE BAKED FOOTPRINT (world v308): the survey's 308 x 233 m record on the west promontory, where the map pin is
  nhm:        { x:-408.0, z:253.0 },   // the two baked footprints south-west of the lagoon, 203 x 151 and 149 x 115 m (world v308)
  teamlab:    { x:-276.0, z:216.0 },   // the district side of the bridge approach, by the interchange (world v309); it stood across the bridge
};
KIT_ZONES[saadiyat.id] = [
  ...
  { x0:-428, x1:-382, z0:236, z1:274 },   // Natural History Museum on its two baked footprints (world v308)
  { x0:-376, x1:-328, z0:284, z1:332 },   // the waterfront park on the peninsula (world v306)
  { x0:-292, x1:-260, z0:204, z1:228 },   // teamLab Phenomena on the district side of the approach (world v309)
...
if (!NO_KIT && saadiyat && kit.louvreAbuDhabi){
  ...
  if (kit.saadiyatGrove){
    ...
    /* CELLS ARE PARCELS OFF THE ROAD NETWORK (world v300). The district's streets are in
       data/roads-saadiyat.json; the old grid ignored them and stood blocks on the carriageways.
       Now the ground is rasterised at half a unit with every road, the museum zones, the beach
       and the lake marked, and blocks are seated only where a whole footprint plus 3 m is
       clear: 86 m blocks on the beach-parallel grid first (11 units, 20 m lanes), then 62, 51
       and 39 m infill in the leftovers. [x, z, w m, d m, kind, storeys]; kind b block, s stone
    ...
    const GROVE_CELLS = [[-376.1,131.5,-0.082,86,86,"b",8],[-362.7,134.1,-0.19,86,86,"b",8],[-349.4,136.7,-0.19,86,86,"b",6],[-336.0,139.2,-0.19,86,86,"b",8],[-338.6,152.6,-0.19,86,86,"b",6],[-322.7,141.8,-0.19,86,86,"b",6],[-338.1,221.9,-0.94,86,86,"b",7],[-322.2,211.2,-0.19,86,86,"b",7],[-290.8,120.2,0.019,86,86,"b",5],